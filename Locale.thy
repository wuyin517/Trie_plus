...
fixes  is_sub:: "'a \<Rightarrow> 'a \<Rightarrow> bool"
...
assumes match_correctness: " \<forall>xl\<in>set (match t l). (lookup t xl) \<and> is_sub xl l"


interpretation FMPM: Match
  where empty = empty_trie and lookup = lookup_trie and insert = insert_trie 
  and delete = delete_trie and invar = invar_trie and to_set = trie_set 
  and is_sub = is_Substring and match = FMPM
